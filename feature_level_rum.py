 ...
 def gen_train_data(self):
        ...
        for line in self.data.values:
            if is_first_line == 1:
                ...
                self.user_sequence_item = dict()
                self.user_sequence_item_number = np.zeros(self.user_number, dtype='int32')
                is_first_line = 0
            else:
                user_id = int(line[0])
                items_id = [i for i in line[1].split('@')]
                size = len(items_id)
                self.user_sequence_item_number[user_id] = size
                self.user_sequence_item[user_id] = [int(alert.split(':')[0]) for alert in items_id]
                for item in items_id:
                    alert = item.split(':')
                    self.train_users.append(int(user_id))
                    self.train_items.append(int(alert[0]))
                    self.train_ratings.append(int(float(alert[1])))
...
def gen_test_data(self):
        ...
        for line in self.data.values:
            if is_first_line == 1:
                ...
                self.user_item_ground_true[user_id] = [int(alert.split(':')[0]) for alert in items_id]

                self.test_batch_real_users.append(int(user_id))
                self.test_batch_real_items.append([int(alert.split(':')[0]) for alert in items_id])
                items += [int(alert.split(':')[0]) for alert in items_id]
...
class rum():
    def __init__(self, data_type):
        ...
        #
        # self.train_user_sequence_items_dict = self.dg.user_purchased_item
        # self.test_user_sequence_items_dict = self.dg.user_item_ground_true
        #
        # self.user_number = self.dg.user_number
        # self.item_number = self.dg.item_number
        # self.neg_number = self.dg.neg_number
        # self.user_sequence_items = self.dg.user_sequence_item
...
def test_AUC(self, loader):
        ...
        while i < 100:  # loader.num_of_step - 5:  # or not q.empty():
            ...
            if len(user_batch) == flow_batch_size:
                _, b = self.sess.run([self.memory, self.log_intention],  # if next
                                     feed_dict={self.user_id: user_batch,
                                                self.item_id: item_batch,
                                                # self.category_id: cate_batch,
        ...
        while i < loader.num_of_step - 5:  # or not q.empty():
            ...
            if q.qsize() < flow_batch_size:
                ...
                q.put((item_part_len[0], 0, tt[0], tt[1], label[0], tt[2]))  # user seq category
                continue
 ...
 def run(self):
        ...
        with open("/home/data/dataset.pkl", 'rb') as f:
...
def save(self):
 ...
 def NDCG_k(self, recommend_list, sequence_list):
        ...
        for i in range(user_number):
            ...
            for j in range(len(recommend_list[i])):
                ...
                if recommend_list[i][j] in sequence_list[i]:
        ...
        for i in range(len(self.test_users) / user_number):
            ...
            for user_index in range(user_index_begin, user_index_end):
                index = [j for j in top_k_index[user_index - user_index_begin] if
                         self.test_candidates[j] not in self.train_user_sequence_items_dict[
                             self.test_users[user_index]]]
                items = [self.test_candidates[j] for j in index]

                pre_top_k.append(list(items[:self.K]))
                ground_truth.append([k for k in self.test_user_sequence_items_dict[self.test_users[user_index]] if
